...
export default function CollectionPage() {
  ...
  const initialFilters = useMemo(() => {
    ...
    return {
      ...
      displayLocation: params.get("displayLocation") || "all",
    };
  }, [search]);
  ...
  useEffect(() => {
    ...
    if (filters.displayLocation && filters.displayLocation !== "all") params.set("displayLocation", filters.displayLocation);
  ...
  const allMaterials = useMemo(() => {
    ...
    return Array.from(new Set(data.artifacts.map((a) => a.material).filter(Boolean))) as string[];
  }, [data?.artifacts]);

  const allRegions = useMemo(() => {
    if (!data?.artifacts) return [];
    return Array.from(new Set(data.artifacts.map((a) => a.region).filter(Boolean))) as string[];
  }, [data?.artifacts]);
