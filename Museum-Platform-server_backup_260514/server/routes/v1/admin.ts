...
router.post("/artifacts", async (req, res) => {
  try {
    ...
    await storage.safeLog({ type: "artifact", title: `New artifact added: ${artifact.title}`, entityId: artifact.id });
...
router.put("/artifacts/:id", async (req, res) => {
  try {
    ...
    await storage.safeLog({ type: "artifact", title: `Artifact updated: ${artifact.title}`, entityId: artifact.id });
    res.json(artifact);
  } catch (error) {
    if (error instanceof z.ZodError) {
      return res.status(400).json({ error: error.errors });
...
router.delete("/artifacts/:id", async (req, res) => {
  try {
    ...
    if (artifact) await storage.safeLog({ type: "artifact", title: `Artifact deleted: ${artifact.title}`, entityId: req.params.id });
    res.status(204).end();
  } catch (error) {
...
router.put("/comments/:id/approve", async (req, res) => {
  try {
    ...
    if (comment) await storage.safeLog({ type: "comment", title: `Comment approved on artifact: ${comment.artifactId}`, entityId: comment.id });
    res.json(comment);
  } catch (error) {
...
router.put("/comments/:id/reject", async (req, res) => {
  try {
    ...
    if (comment) await storage.safeLog({ type: "comment", title: `Comment rejected on artifact: ${comment.artifactId}`, entityId: comment.id });
    res.json(comment);
  } catch (error) {
...
router.delete("/users/:id", async (req, res) => {
  try {
    ...
    if (user) await storage.safeLog({ type: "user", title: `User deleted: ${user.email}`, entityId: req.params.id });
    res.status(204).end();
  } catch (error) {
