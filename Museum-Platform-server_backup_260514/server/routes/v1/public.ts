...
router.get("/artifacts", async (req, res) => {
  try {
    console.log("inside artifacts route, query:");
    const userId = req.session.userId!;
    const options = {
      search: req.query.search as string | undefined,
      categoryId: req.query.category as string | undefined,
      displayLocation: req.query.displayLocation as string | undefined,
    ...
    if (!userId) { options.status = "published"; };
    console.log("Artifact query options:", options, "status", req.query.status);
...
router.get("/artifacts/:id", async (req, res) => {
  try {
    ...
    if (!userId && artifact.status === "draft") {
      return res.status(404).json({ error: "Artifact not found" });
    }
