...
export interface IStorage {
  ...
  logActivity(data: NewActivityLog): Promise<ActivityLog>;
...
export class DatabaseStorage implements IStorage {
  ...
  // safe wrapper — put this in storage.ts or a utils file
  async safeLog(data: NewActivityLog) {
    try {
      await storage.logActivity(data);
    } catch (err) {
  ...
  async getRecentActivity(limit = 10): Promise<RecentActivityDTO[]> {
    ...
    return rows.map((row) => ({
      ...
      type: row.type,
      title: row.title,
      date: this.formatActivityDate(row.createdAt),
    }));
  }
  ...
  //  async verifyPasswordAndRole(email: string, password: string): Promise<User | null> {
  ...
  async getArtifacts(options?: {
    ...
    if (displayLocation && displayLocation !== "all") {
      conditions.push(eq(artifacts.displayLocation, displayLocation as "in_museum" | "returned"));
    }
  ...
  // Add this method to your storage.ts file

  // async getStatistics() {
  //   try {
  //     // Get total number of published artifacts
  //     const totalArtifactsResult = await db.query(
  //       `SELECT COUNT(*) as count FROM artifacts WHERE status = 'published'`
  //     );
  //     const totalArtifacts = parseInt(totalArtifactsResult.rows[0].count);

  //     // Get total number of unique regions
  //     const totalRegionsResult = await db.query(
  //       `SELECT COUNT(DISTINCT region) as count
  //      FROM artifacts
  //      WHERE status = 'published' AND region IS NOT NULL`
  //     );
  //     const totalRegions = parseInt(totalRegionsResult.rows[0].count);

  //     // Get total number of views (sum of all artifact views)
  ...
  //     // Calculate years of history (from earliest to latest year in artifacts)
  ...
  //     const earliestYear = yearsResult.rows[0].earliest_year;
  //     const latestYear = yearsResult.rows[0].latest_year;
  //     const yearsOfHistory = earliestYear && latestYear
  //       ? Math.abs(latestYear - earliestYear)
  ...
  async getStatistics(): Promise<{
    ...
    const [artifactStats] = await db
      .select({
        ...
        totalViews: sql<number>`COALESCE(SUM(${artifacts.views}), 0)`,
        totalRegions: sql<number>`COUNT(DISTINCT ${artifacts.region})`,
      })
      .from(artifacts)
      .where(eq(artifacts.status, "published"));
    ...
    return {
      ...
      totalRegions: Number(artifactStats.totalRegions),
      totalViews: Number(artifactStats.totalViews) || 0,
    };
  }
}
